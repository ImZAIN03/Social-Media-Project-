  ...
  return (
    <div className='w-screen h-screen flex items-center justify-center'>
      <div className='flex items-center justify-evenly w-[80%]'>
        <div>
          <img className='ml-1' width={"180px"} src='SM.png' alt='Logo'/>
        </div>
      <div >
        <div className='my-5'>
        ...
        <form className='flex flex-col w-[70%]'>
          ...
          <button className='px-4 py-2 text-lg border-none bg-cyan-600 text-white rounded-full my-3'>Login</button>
          <h1>{isLogin ? "Don't have and account?" :  "Already have and account?"} <span className='cursor-pointer' onClick={loginSignupHandler}>{isLogin ? "Register" :"Login"}</span></h1>
        </form>
      </div>
      </div>
    </div>
